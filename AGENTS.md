...
- Symlink containment is part of the workspace boundary invariant: both the index walk (`src/files.ts`) and the send-time mention check (`src/mention.ts`) resolve each link with `realpath` and drop any target that resolves outside the canonical workspace root, so a planted `notes -> ~/.ssh` alias can neither be indexed nor injected.
